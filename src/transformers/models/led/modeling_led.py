...
class LEDEncoder(LEDPreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        ...
        self.layernorm_embedding = self.layernorm_embedding.to(self.first_device)
        self.layer_device_dict = {}
        for k, v in self.device_map.items():
            for layer_i in v:
                cuda_device = "cuda:" + str(k)
                self.layers[layer_i] = self.layers[layer_i].to(cuda_device)
                self.layer_device_dict[layer_i] = cuda_device
    ...
    def deparallelize(self):
        ...
        self.model_parallel = False
        self.embed_positions = self.embed_positions.to("cpu")
        self.layernorm_embedding = self.layernorm_embedding.to("cpu")
        for layer_i in range(num_layers):
            self.layers[layer_i] = self.layers[layer_i].to("cpu")
            self.layer_device_dict[layer_i] = "cpu"
    ...
    def forward(
        ...
        if global_attention_mask is not None:
            if self.model_parallel:
                global_attention_mask = global_attention_mask.to(self.first_device)
...
class LEDForConditionalGeneration(LEDPreTrainedModel):
    ...
    @add_start_docstrings("")
    def parallelize(self, device_map=None):
        num_layers_each = len(self.led.encoder.layers)
        num_layers = num_layers_each * 2  # encoder + decoder
        self.device_map = (
            get_device_map(num_layers, range(torch.cuda.device_count()))
            if device_map is None
            else device_map
        ...
        # embedding
        self.led.shared = self.led.shared.to(self.first_device)
        self.led.encoder.parallelize()
        self.led.decoder.parallelize()
        self.lm_head = self.lm_head.to(self.last_device)
        self.model_parallel = True
...
@add_start_docstrings(
    """
    LED model for Multiple Choice
    """,
    LED_START_DOCSTRING,
)
class LEDForMultipleChoice(LEDPreTrainedModel):
    def __init__(self, config: LEDConfig, **kwargs):
        super().__init__(config, **kwargs)
        self.led = LEDModel(config)
        self.classification_head = LEDClassificationHead(
    ...
    def parallelize(self, device_map=None):
        ...
        self.led.encoder.parallelize()
        self.led.decoder.parallelize()
        self.classification_head = self.classification_head.to(self.last_device)
        self.model_parallel = True
    ...
    def forward(
        ...
        for choice_i in range(num_choices):
            ...
            eos_mask = input_ids[:, choice_i].eq(self.config.eos_token_id)
            ...
            example_logits = self.classification_head(sentence_representation)
            logits_list.append(example_logits)
