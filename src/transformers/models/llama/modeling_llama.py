# coding=utf-8
# Copyright 2022 The FAIR team of Meta AI The HuggingFace Inc. team. All rights reserved.
#
...
def load_tf_weights_in_llama(model, config, tf_checkpoint_path):
...
class LLaMASelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in LLaMAModel forward() function)
...
class LLaMASelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LLaMAAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = LLaMASelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = LLaMASelfOutput(config)
...
class LLaMAIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LLaMALayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = LLaMAAttention(config, position_embedding_type="absolute")
        self.intermediate = LLaMAIntermediate(config)
        self.output = LLaMAOutput(config)
...
class LLaMAEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([LLaMALayer(config) for _ in range(config.num_hidden_layers)])
...
class LLaMAPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_llama
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, LLaMAEncoder):
...
LLAMA_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            ...
            Indices can be obtained using [`LLaMATokenizer`].
...
class LLaMAModel(LLaMAPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.encoder = LLaMAEncoder(config)
...
@add_start_docstrings("""LLaMA Model with a `language modeling` head on top. """, LLAMA_START_DOCSTRING)
...
class LLaMAForCausalLM(LLaMAPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `LLaMAForCausalLM` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        >>> model = LLaMAForCausalLM.from_pretrained('llama-7b', config=config)
...
@add_start_docstrings(
    """LLaMA Model transformer with a sequence classification/regression head on top (a linear layer on top of
...
@add_start_docstrings(
    """LLaMA Model with a multiple choice classification head on top (a linear layer on top of
...
class LLaMAForMultipleChoice(LLaMAPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(LLAMA_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        checkpoint=_CHECKPOINT_FOR_DOC,
...
@add_start_docstrings(
    """LLaMA Model with a token classification head on top (a linear layer on top of
...
@add_start_docstrings(
    """LLaMA Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
