...
from glue_lm.runners import GlueLMTaskRunner, RunnerParameters
import glue_lm.model_setup as glue_lm_model_setup
...
def get_args(*in_args):
    ...
    parser.add_argument("--glue_loss_weight",
                        default=1.,
                        type=float)
...
def main():
    ...
    glue_model = glue_model_setup.create_model(
    ...
    glue_lm_model = glue_lm_model_setup.BertGlueLM(
    ...
    if args.do_train:
        if args.print_trainable_params:
            ...
            print("  SHARED:")
            for param_name, param in glue_model.bert.named_parameters():
                if param.requires_grad:
                    print("    {}  {}".format(param_name, tuple(param.shape)))
            print("  GLUE:")
            for param_name, param in glue_model.named_parameters():
                if param.requires_grad and not param_name.startswith("bert."):
                    print("    {}  {}".format(param_name, tuple(param.shape)))
            print("  LM:")
            for param_name, param in lm_model.named_parameters():
                if param.requires_grad and not param_name.startswith("bert."):
                    print("    {}  {}".format(param_name, tuple(param.shape)))
    ...
    runner = GlueLMTaskRunner(
        ...
        rparams=RunnerParameters(
            ...
            glue_loss_weight=args.glue_loss_weight, lm_loss_weight=args.lm_loss_weight,
